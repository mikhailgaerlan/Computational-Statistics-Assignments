...
for(i in 1:n){
    sorted = sort(yboot[,i])
    upperband[i] = sorted[ceiling(interval*B)]
    lowerband[i] = sorted[floor((1-interval)*B)]
}
...
limits = read.csv("limits2.csv")
limits = limits$X
heights = read.csv("heights2.csv")
heights = heights$X
ymodel = getmodel(limits,heights)
...
pdf(file = "../Graphs/confidenceresid2.pdf",height=4,width=3.5,family="Times New Roman")
plot(x,y,ylim=c(min(lowerband),max(upperband)),main="Bootstrap Residuals",xlab=expression(italic(x)),ylab=expression(italic(y)))
lines(x,f)
lines(x,ymodel,col='blue')
lines(x,upperband,col='green')
