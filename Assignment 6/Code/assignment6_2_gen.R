...
set.seed(0401)
y = f+rnorm(f)/3
pdf(file = "../Graphs/genetic.pdf",height=4,width=3.5,family="Times New Roman")
...
write("",file="limits.csv",append=FALSE)
write("",file="heights.csv",append=FALSE)
#Best Model
for (i in 1:pop_size){
  if (rankings[i] == pop_size){
    chromo = population[i,1:(n-1)]
    ...
    ymodel = getmodel(limits,heights)
    for(limit in limits){
        write(sprintf("%f\n",limit),file="limits.csv",append=TRUE)
    }
    for(height in heights){
        write(sprintf("%f\n",height),file="heights.csv",append=TRUE)
    }
    plot(x,y)
