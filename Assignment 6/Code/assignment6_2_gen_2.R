...
set.seed(0401)
y = f+rnorm(f)/5
pdf(file = "../Graphs/genetic2.pdf",height=4,width=3.5,family="Times New Roman")
plot(x,y,main="",xlab=expression(italic(x)),ylab=expression(italic(f(x))))
lines(x,f)
dev.off()
...
#FALSE for AIC
if (criteria){
  file_name = sprintf("mdl_gen_data_%d_%d2.csv",pop_size,stop_gen)
} else{
  file_name = sprintf("aic_gen_data_%d_%d2.csv",pop_size,stop_gen)
}
...
write("",file="limits2.csv",append=FALSE)
write("",file="heights2.csv",append=FALSE)
#Best Model
for (i in 1:pop_size){
  if (rankings[i] == pop_size){
    chromo = population[i,1:(n-1)]
    ...
    ymodel = getmodel(limits,heights)
    for(limit in limits){
        write(sprintf("%f\n",limit),file="limits2.csv",append=TRUE)
    }
    for(height in heights){
        write(sprintf("%f\n",height),file="heights2.csv",append=TRUE)
    }
    plot(x,y)
